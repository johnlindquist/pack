...
describe("E2E: Config file", () => {
  ...
  beforeAll(async () => {
    testDir = await fs.mkdtemp(path.join(os.tmpdir(), "packx-config-test-"));
...
`);
    ...
    await fs.writeFile(path.join(testDir, "lib.ts"), `
// Library code
export function lib() {
  // FIXME: broken
  return 42;
...
`);
  ...
  test("init creates config template", async () => {
    const { code } = await runCLI(["init", "test-config"], testDir);
    expect(code).toBe(0);

    const configPath = path.join(testDir, "test-config.ini");
  ...
  test("uses config file with -f", async () => {
    // Create a config file
    const configPath = path.join(testDir, "search.ini");
    await fs.writeFile(configPath, `[search]
TODO
FIXME
...
`);
    ...
    const { stdout, code } = await runCLI(["-f", "search.ini", "--preview"], testDir);
    expect(code).toBe(0);
